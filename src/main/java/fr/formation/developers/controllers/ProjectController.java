...
public class ProjectController {
    ...
    // (mandatory, length of character,at least one character, date at the present
    // or at the future and optionnal (if not optionnal >0)
    @PostMapping
    public void createProject(@Valid @RequestBody ProjectCreate projectCreate) {
	System.out.println("The project of name " + projectCreate.getName() + " has a description "
		+ projectCreate.getDescription() + " with the date of beginning " + projectCreate.getDateBegin()
		+ " and with annual money of " + projectCreate.getMoneyAnnual() + " euros");

    }

    // Modify the fields of the project by the name with rules on fields
    // "description" and "moneyAnnual" of the object
    // "projectUpdate " with type "ProjectUpdate"
    // (mandatory, length of character,at least one character, optionnal, if not
    // optionnal moneyAnnual >0)
    ...
    // "projectClose" with type "ProjectClose"
    // (mandatory, date at the future)
